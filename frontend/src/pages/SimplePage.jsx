...
import { faculty as fallbackFaculty, news } from '../data/siteData';
  ...
  return (
    <>
      ...
      <section className="container-editorial page-band">
        {type === 'team' && <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{faculty.map((person, index) => <div className="scroll-reveal" style={{ transitionDelay: `${index * 80}ms` }} key={person.id || person.name}><FacultyCard person={person} /></div>)}</div>}
