          ...
          <div className="mx-auto grid max-w-xl gap-2">
            {navItems.map((item) => (
              <div key={item.path} className="border-b border-slate-200 py-3">
                <Link className="block font-editorial text-3xl font-semibold text-ink" to={item.path} onClick={() => setOpen(false)}>
                  {item.label}
                </Link>
                {item.children && (
                  <div className="mt-3 grid gap-2 pl-3">
                    {item.children.map((child) => (
                      <Link className="py-1 text-sm font-semibold uppercase tracking-[0.14em] text-slate-500" key={child.path} to={child.path} onClick={() => setOpen(false)}>
                        {child.label}
                      </Link>
                    ))}
                  </div>
            ...
            <div className="mt-8 grid grid-cols-[auto_1fr] gap-3">
              <button className="grid h-12 w-12 place-items-center border border-slate-200 text-ink" aria-label="Search"><Search size={18} /></button>
              <Link className="grid place-items-center rounded bg-ink px-5 py-4 text-sm font-bold uppercase tracking-[0.18em] text-white" to="/admissions" onClick={() => setOpen(false)}>Apply Now</Link>
            </div>
          </div>
        </div>
      )}
