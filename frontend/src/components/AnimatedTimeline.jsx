...
export default function AnimatedTimeline({ items }) {
  return (
    <div className="relative grid gap-6 border-l-2 border-gold/40 pl-7">
      {items.map((item) => (
        <motion.div initial={{ opacity: 0, x: -18 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="theme-panel relative p-6" key={item.title}>
          ...
          <h3 className="mt-2 font-editorial text-2xl font-semibold text-ink">{item.title}</h3>
